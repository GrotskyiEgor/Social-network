...
def get_friendship_requests(current_user):
    return (
        Profile.objects.
        filter(sent_friendships__to_user=current_user, sent_friendships__status='pending').
        order_by("id")
    )

def get_friendship_recommendation(current_user):
    ...
    return Profile.objects.exclude(id__in=busy_ids).order_by("id")
...
def get_friends(current_user):
    ...
    return Profile.objects.filter(id__in=friend_ids).order_by("id")